...
@section('content')
  <div class="w-full lg:w-1/2 mx-auto rounded-xl bg-white p-3 m-3">
   ...
   <form action="{{route('post.update')}}" method="POST" enctype="multipart/form-data">
    ...
    <input type="text" name="title" value="{{$post->title}}" class="text-xl text-first my-2 w-full text-xl p-2 rounded-xl border-2 border-gray-300">
    <label for="description" class="text-xl">Description:</label>
     <textarea name="description" value="{{$post->description}}" class="p-2 text-xl text-first w-full h-28 rounded-xl border-2 border-gray-400 my-2"></textarea>
     <img src="/storage/post/{{$post->photo}}" class="w-48 m-2">
